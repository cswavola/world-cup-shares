...
function eliminatedTeams(state) {
  ...
  // Only each team's most advanced knockout match matters here — an earlier
  // round win shouldn't keep a team "active" after they've since lost a later one.
  const latestKO = {};
  for (const m of state.matches) {
  ...
  for (const code in latestKO) {
    ...
    else if (m.stage === "sf") hasUpcoming.add(code); // sf loser stays active for the third-place match
  }
...
// Stage a knockout winner moves into next, once they've won their current match.
...
function teamLastRound(state) {
  ...
  for (const m of state.matches) {
    bump(m.a, m.stage);
    bump(m.b, m.stage);
    // Promote the winner into the next round immediately, rather than waiting
    // for the live feed to resolve their next opponent's name.
    if (m.stage === "sf") {
      if (m.outcome === "a") { bump(m.a, "final"); bump(m.b, "third"); }
      else if (m.outcome === "b") { bump(m.b, "final"); bump(m.a, "third"); }
